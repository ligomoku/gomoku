    ...
    const submit = (v, submitOpts) => {
        ...
        if (v.length > 0 && 'resign'.startsWith(v.toLowerCase())) {
            ...
            if (v.toLowerCase() === 'o-o' && legalSans['O-O-O'] && !submitOpts.force)
                return;
            // ambiguous promotion
            if (isKey(v) && (selectedKey + v).match(ambiguousPromotionRegex) && !submitOpts.force)
                return;
            // ambiguous UCI
            if (isKey(v) && selectedKey)
                opts.ctrl.select(v);
            ...
            if ('next' === v.toLowerCase()) {
                (_b = (_a = opts.ctrl).next) === null || _b === void 0 ? void 0 : _b.call(_a);
                clear();
            }
        }
        else if (v.length > 0 && 'upv'.startsWith(v.toLowerCase())) {
            if ('upv' === v.toLowerCase()) {
                (_d = (_c = opts.ctrl).vote) === null || _d === void 0 ? void 0 : _d.call(_c, true);
                clear();
            }
        }
        else if (v.length > 0 && 'downv'.startsWith(v.toLowerCase())) {
            if ('downv' === v.toLowerCase()) {
                (_f = (_e = opts.ctrl).vote) === null || _f === void 0 ? void 0 : _f.call(_e, false);
                clear();
            }
        }
...
function iccfToUci(v) {
    const chars = v.split('');
    if (chars[0])
        chars[0] = files[parseInt(chars[0]) - 1];
    if (chars[2])
        chars[2] = 'kqrbn'[parseInt(chars[2])];
    return chars.join('');
}
...
function readOpponentName() {
    const opponentName = document.querySelector('.ruser-top');
