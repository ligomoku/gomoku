...
import type { UseMutationOptions } from "@tanstack/react-query";
 ...
 type PostApiGameRegisteredGameidJoinClient = typeof client<PostApiGameRegisteredGameidJoinMutationResponse, PostApiGameRegisteredGameidJoin404, never>;
 ...
 */
export function usePostApiGameRegisteredGameidJoin(gameId: PostApiGameRegisteredGameidJoinPathParams["gameId"], headers: PostApiGameRegisteredGameidJoin["headerParams"], options: {
    mutation?: UseMutationOptions<PostApiGameRegisteredGameidJoin["response"], PostApiGameRegisteredGameidJoin["error"], PostApiGameRegisteredGameidJoin["request"]>;
    client?: PostApiGameRegisteredGameidJoin["client"]["parameters"];
} = {}) {
    const { mutation: mutationOptions, client: clientOptions = {} } = options ?? {};
    return useMutation({
        mutationFn: async () => {
            const res = await client<PostApiGameRegisteredGameidJoin["data"], PostApiGameRegisteredGameidJoin["error"], PostApiGameRegisteredGameidJoin["request"]>({
                method: "post",
