...
import type {
  PostGamehubRegisteredRequestundoMutationResponse,
  PostGamehubRegisteredRequestundoQueryParams,
} from "../models/PostGamehubRegisteredRequestundo";
import type { UseMutationOptions } from "@tanstack/react-query";

type PostGamehubRegisteredRequestundoClient = typeof client<
  PostGamehubRegisteredRequestundoMutationResponse,
  Error,
  never
>;
 ...
 */
export function usePostGamehubRegisteredRequestundo(
  params?: PostGamehubRegisteredRequestundo["queryParams"],
  options: {
    mutation?: UseMutationOptions<
      PostGamehubRegisteredRequestundo["response"],
      PostGamehubRegisteredRequestundo["error"],
      PostGamehubRegisteredRequestundo["request"]
    >;
  ...
  return useMutation({
    mutationFn: async () => {
      const res = await client<
        PostGamehubRegisteredRequestundo["data"],
        PostGamehubRegisteredRequestundo["error"],
        PostGamehubRegisteredRequestundo["request"]
      >({
        method: "post",
