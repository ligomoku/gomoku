...
import type { UseMutationOptions } from "@tanstack/react-query";
 ...
 type PostGamehubRegisteredApproveundoClient = typeof client<PostGamehubRegisteredApproveundoMutationResponse, Error, never>;
 ...
 */
export function usePostGamehubRegisteredApproveundo(params?: PostGamehubRegisteredApproveundo["queryParams"], options: {
    mutation?: UseMutationOptions<PostGamehubRegisteredApproveundo["response"], PostGamehubRegisteredApproveundo["error"], PostGamehubRegisteredApproveundo["request"]>;
    client?: PostGamehubRegisteredApproveundo["client"]["parameters"];
} = {}) {
    const { mutation: mutationOptions, client: clientOptions = {} } = options ?? {};
    return useMutation({
        mutationFn: async () => {
            const res = await client<PostGamehubRegisteredApproveundo["data"], PostGamehubRegisteredApproveundo["error"], PostGamehubRegisteredApproveundo["request"]>({
                method: "post",
