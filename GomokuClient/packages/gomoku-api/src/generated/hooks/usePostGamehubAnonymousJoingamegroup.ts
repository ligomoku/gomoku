...
import type {
  PostGamehubAnonymousJoingamegroupMutationResponse,
  PostGamehubAnonymousJoingamegroupQueryParams,
} from "../models/PostGamehubAnonymousJoingamegroup";
import type { UseMutationOptions } from "@tanstack/react-query";

type PostGamehubAnonymousJoingamegroupClient = typeof client<
  PostGamehubAnonymousJoingamegroupMutationResponse,
  Error,
  never
>;
 ...
 */
export function usePostGamehubAnonymousJoingamegroup(
  params?: PostGamehubAnonymousJoingamegroup["queryParams"],
  options: {
    mutation?: UseMutationOptions<
      PostGamehubAnonymousJoingamegroup["response"],
      PostGamehubAnonymousJoingamegroup["error"],
      PostGamehubAnonymousJoingamegroup["request"]
    >;
  ...
  return useMutation({
    mutationFn: async () => {
      const res = await client<
        PostGamehubAnonymousJoingamegroup["data"],
        PostGamehubAnonymousJoingamegroup["error"],
        PostGamehubAnonymousJoingamegroup["request"]
      >({
        method: "post",
